...
run_comparisons_over_n <- function(all_n, d, num_repeats_full, savefilename) {
    ...
    for (nind in 1:num_n) {
        for (irep in 1:num_repeats_full) {
            ...
            # Yuan and Samworth estimator via logcondens
            t_start <- Sys.time()
            algo_output <- yuan_samworth_alternating_with_repeats(SimData, num_repeats=10, max_iter=20)
            t_end <- Sys.time()
...
for (d in c(4)) {
    savefilename <- sprintf('Results/yuan_samworth_over_n_d=%s.RData', d)
